...
class WalletController extends Controller
{
    ...
    public function deposit(Request $request)
    {
        ...
        else
        {
            ...
            elseif($request->selected_payment_gateway === 'paystack'){
                try {
                    return PaymentGatewayRequestHelper::paystack()->charge_customer($this->buildPaymentArg($total,$title,$description,$last_deposit_id,$email,$name,route('freelancer.paystack.ipn.wallet')));
                }catch (\Exception $e){
                    toastr_error($e->getMessage());
                    return back();
                }
    ...
    private function buildPaymentArg($total,$title,$description,$last_deposit_id,$email,$name,$ipn_route)
    {
        return [
            ...
            'success_url' => route('freelancer.wallet.history'),
            'email' => $email,
            'name' => $name,
