...
class WalletController extends Controller
{
    ...
    public function deposit(Request $request)
    {
        ...
        if($request->selected_payment_gateway === 'manual_payment') {
            ...
            elseif($request->selected_payment_gateway === 'paystack'){
                try {
                    return PaymentGatewayRequestHelper::paystack()->charge_customer($this->buildPaymentArg($total,$title,$description,$last_deposit_id,$email,$name,route('client.paystack.ipn.wallet')));
                }catch (\Exception $e){
                    toastr_error($e->getMessage());
                    return back();
                }
    ...
    private function buildPaymentArg($total,$title,$description,$last_deposit_id,$email,$name,$ipn_route)
    {
        return [
            ...
            'success_url' => route('client.wallet.history'),
            'email' => $email,
            'name' => $name,
