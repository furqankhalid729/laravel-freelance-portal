        ...
        $(document).ready(function () {
            ...
            $(document).on('change', '#country , #category , #level , #talent_badge, #skill', function() {
                ...
                $.ajax({
                    ...
                    method:'GET',
                    data:{country:country,category:category,level:level,talent_badge:talent_badge,skill:skill},
            ...
            $(document).on('click', '.pagination a', function(e){
                ...
                let level = $('#level').val();
                let skill = $('#skill').val();
                jobs(page,country,talent_badge,category,level,skill);
            });
            function jobs(page,country,talent_badge,category,level,skill){
                $.ajax({
                    url:"{{ route('talents.pagination').'?page='}}" + page,
                    method:'GET',
                    data:{country:country,talent_badge:talent_badge,level:level,category:category,skill:skill},
                    success:function(res){
                        if(res.status=='nothing'){
